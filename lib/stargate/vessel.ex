defmodule Stargate.Vessel do
  ...
  import Vessel.Websocket, only: [handle_ws_handshake: 2]
  ...
  def on_tcp(config = %{state: :ws}, bin) do
    ...
    payload = Vessel.Websocket.Frame.parse_frame(buf)
    IO.inspect(payload)

    config.transport.send(
      config.socket,
      Vessel.Websocket.Frame.format_server_frame("Hi there", :text)
    )
  ...
  def on_tcp(config, bin) do
    ...
    cond do
      ...
      end_of_header? == :nomatch ->
        Map.put(config, :buf, buf)
